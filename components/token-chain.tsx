...
import { Link2, User, KeyRound, Bot, Zap, ArrowDown, CheckCircle2, Clock } from "lucide-react"
  ...
  return (
    <div className="flex items-center gap-2 py-1 pl-3">
      <div className="flex flex-col items-center gap-0.5">
        <div className={cn("w-px flex-1 bg-border/60", active && "bg-primary/40")} style={{ height: 8 }} />
      ...
      <span
        className={cn(
          "font-mono text-[10px]",
          highlight ? "text-violet-500 font-semibold" : "text-muted-foreground",
          !active && "opacity-50",
        )}
      >
        {label}
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          {node && (
            <span className="font-mono text-[9px] text-muted-foreground">{node.ts}</span>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-2">
      ...
      <ScrollArea className="rounded-lg border border-border bg-card/40 p-3" style={{ maxHeight: 520 }}>
        <div className="flex flex-col gap-1">
          ...
          {/* ① OIDC Session */}
          <ChainNode num={1} label="OIDC Session" type="id_token" icon={User} node={oidc}>
          ...
          {/* ② Access Token */}
          <ChainNode num={2} label="Access Token" type="Bearer" icon={KeyRound} node={access}>
            <div className="flex flex-col gap-0.5">
              <Claim
                label="scope"
                value={(access?.scopes ?? []).slice(0, 4).join(" ")}
              />
            </div>
          </ChainNode>
          ...
          {/* ③ Agent Delegated Token */}
          <ChainNode
            num={3}
            label="Agent Delegated Token"
            type="OBO"
            icon={Bot}
            node={obo}
            waitingLabel="Fires on first tool call"
          >
            <div className="flex flex-col gap-0.5">
          ...
          <ChainNode
            ...
            icon={Zap}
            node={stepUp}
            waitingLabel="Fires on CIBA approval"
          >
            <div className="flex flex-col gap-0.5">
